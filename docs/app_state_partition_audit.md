...
- campaign：`activeCampaign`、`selectedCampaign`、`frontendState`、`lastCampaignState`、`assetSeed`、`streamingPreview`
...
- character：`latestInventoryPayload`、`latestDossierPayload`
- writeback：`writebackReview`
- job：`runProgress`、`polling`、`lastJobRunning`
- ui：`sideTab`、`activePanel`、`autoScroll`、`collapsedPanels`、`dragPanels`、`activeDragPanel`
...
`LEGACY_STATE_FIELDS` 仍用于兼容旧调用路径，避免后续未收敛代码或浏览器缓存中的旧入口直接崩溃。后续迁移应继续优先使用分区字段，并在确认没有旧入口后再考虑删除 proxy。
...
1. 继续收敛未纳入本轮的 legacy 字段，例如 `characterProfileExpanded`、`characterProfileCampaign`、`ruleFiles`、`selectedRule`。
