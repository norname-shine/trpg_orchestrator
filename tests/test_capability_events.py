...
def test_legacy_keyword_requests_still_trigger_capabilities():
    ...
    assert "dossier" in _capabilities("查看资料和线索")
...
def test_image_negated_does_not_trigger_visual_assets():
    ...
    assert plan["output_contract"]["allow_gallery_update"] is False
    assert plan["frontend_refresh"]["gallery"] == "no_update"
...
def test_item_inspected_event_triggers_inventory_and_dossier():
    capabilities = _capabilities("仔细检查刚获得的徽章")

    assert "inventory" in capabilities
    assert "dossier" in capabilities
...
def test_clue_observed_event_triggers_dossier():
    assert "dossier" in _capabilities("注意到墙上的血色符号")
